		...
		if(!isParallel)
		{
			...
			while((token = strtok_r(ptr,";",&rest))!= NULL )
			{
				...
				{ 
					...
					if(redirections!=1 || strchr(restRedir,'>') || strchr(restRedir,'|'))//wrong redirection syntax
					{
						print_error_message();
						break;
					}
				...
				int retval=0;// =-1;
				if(strchr(tokenizeRedir,'|')==NULL)
				{
					retval = handleCommandSequential(tokenizeRedir);
				}
				else
				{
					...
					if(pid == 0)
					{
						...
						char* words[512];
						int word_count = split_line(xx,words);	
						if(word_count == 0) exit(0);
					...
					{
						pid_t pid2=-1;
						if(retval==0) pid2 = fork();
						if(pid2 == 0)
						{
							close(fds[1]);
							dup2(fds[0],STDIN_FILENO);
							char* words[512];
							int word_count = split_line(yy,words);	
							if(word_count == 0) exit(0);
					...
					{
						char* zz=NULL, *ww=NULL;
						zz=strtok_r(yy,"|",&ww);
						pid_t pid3 = -1;
						if(retval == 0) pid3=fork();
						if(pid3 == 0)
						{
							close(fds[1]);
							dup2(fds[0],STDIN_FILENO);
							...
							char* words[512];
							int word_count = split_line(zz,words);	
							if(word_count == 0) exit(0);
							char* args[513];
							int i;
							for(i=0;i<word_count;++i)
						...
						if(pid4 == 0)
						{
							...
							char* words[512];
							int word_count = split_line(ww,words);	
							if(word_count == 0) exit(0);
