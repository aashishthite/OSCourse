...
//char* built_in_commands[]={"cd","pwd","quit"};
//FILE* inputFile=NULL;
...
void decodeAndExecuteCommand(FILE* inFile)
{
	...
	while(fgets(input,INP_SIZE,inFile)!=NULL)
	{
		...
		if(!isParallel)
		{
			...
			while((token = strtok_r(ptr,";",&rest))!= NULL )
			{
				...
				{
					print_error_message();
					print_debug_message("cannot execute\n");
					continue;
					//goto endofwhile;
				}
		...
		if(isParallel)
		{
			...
			while((token = strtok_r(ptr,"+",&rest))!= NULL )
			{
				...
				{ 
					...
					//TODO	
					close(STDOUT_FILENO);
					int fd = open(words_redir[0],O_CREAT | O_TRUNC |O_WRONLY,S_IRUSR | S_IRGRP | S_IWGRP | S_IWUSR);
					if(fd==-1)
					{
						print_error_message();
						break;
	...
	//				clearerr(stdout);
		...
		char* words[512];
		int i=0;
		int num = split_line(input,words);
		print_command(input);
		char* outFileName;
		char* copy_of_input = strdup(input);
		char* token = strtok(copy_of_input,">");
		if(strlen(token) != strlen(input))
		{
			redirected=1;	
		...
		if(redirected)
		{
			word_count = split_line(token, words);
			int numFiles = split_line(outFileName, words+word_count);
			if(numFiles!=1)
			{
				print_error_message();
				goto endofwhile;
